...
public class ImageService {
	...
	public Image mark(Image image, String token) throws IOException {
		...
		// ApiConstant.CLASSIFICATOR_TRAINING_MARKED_DIR_REMOTE + image.getName());
	 ...
	 * 
	 */
	public Image pick_image(String token) {
		...
		while (diff < ApiConstant.IMAGE_SEARCH_WAITING_TIME) {
			...
			// get the list of all sub folders assuming that there are only folder there
			List<File> notMarkedDirFiles = Arrays.asList(notMarkedDir.listFiles());

			// generate a number between 0 and the number of sub folders minus one to index
	...
	public boolean asAlreadyMarked(String name, String userId) {
